...
**37 core modules + 32 v2 constitutional modules. 1104 tests. Zero heavy dependencies. Running code. MCP server included.**
...
**72 tools across all 37 modules, role-scoped access control.** Identity, delegation, agora, values/policy, coordination, and commerce — all accessible via MCP. Every operation Ed25519 signed. Auto-configures Claude Desktop and Cursor.
...
```bash
npm test
# 1104 tests across 58 files, 295 suites, 0 failures
```
...
tests/                  59 test files, 1104 tests (295 suites)
