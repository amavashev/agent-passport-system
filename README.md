...
**42 core modules + 32 v2 constitutional modules. 1241 tests. Zero heavy dependencies. Running code. MCP server included.**
...
**83 tools across all 42 modules, role-scoped access control.** Identity, delegation, agora, values/policy, coordination, and commerce — all accessible via MCP. Every operation Ed25519 signed. Auto-configures Claude Desktop and Cursor.
...
```bash
npm test
# 1241 tests across 58 files, 332 suites, 0 failures
```
...
tests/                  65 test files, 1241 tests (332 suites)
