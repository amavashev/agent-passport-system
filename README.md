...
**37 core modules + 32 v2 constitutional modules. 1165 tests. Zero heavy dependencies. Running code. MCP server included.**
...
**82 tools across all 41 modules, role-scoped access control.** Identity, delegation, agora, values/policy, coordination, and commerce — all accessible via MCP. Every operation Ed25519 signed. Auto-configures Claude Desktop and Cursor.
...
```bash
npm test
# 1165 tests across 58 files, 316 suites, 0 failures
```
...
tests/                  62 test files, 1165 tests (316 suites)
